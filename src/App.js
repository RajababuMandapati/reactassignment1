...
class HistoryListApp extends Component {
  ...
  render() {
    ...
    const searchResults = historyList.filter(eachHistoryItem =>
      eachHistoryItem.title.lowerCase().includes(searchInput.lowerCase()),
    )
    return (
            ...
            <ul className="history-list">
              {searchResults.map(eachHistoryItem => (
                <UseHistoryList
                  ...
                  onDeleteHistoryItem={thisonDeleteHistoryItem}
                />
              ))}
            </ul>
          ) : (
            <p className="emty-history-message">There is no history</p>
          )}
        </div>
      </div>
    )
